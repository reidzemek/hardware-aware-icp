...
out_dir_LIGHT = 'P2P_ICP_3D_Figures_LIGHT';
out_dir_DARK = 'P2P_ICP_3D_Figures_DARK';

%% Generate an irregular smooth squiggly surface

% Parameters
range = 100;            % size of 3D cubic volume containing surface (must be a whole number multiple of 4)
...
downsample_factor = 1;  % source point cloud downsample factor

% Initizlize time singal vector for horizontal axes (x and y)
...
% Stocastically generate both horizontal functions
for k = 1:num_terms
    A1 = ((rand()*(sqrt(range/2)/3))+(2*(sqrt(range/2)/3)))*(1/k);
    ...
    A2 = ((rand()*(sqrt(range/2)/3))+(2*(sqrt(range/2)/3)))*(1/k);
...
% Downsample the surface to generate a 3D point cloud
pc = zeros((n_points*4*extent+1)^2, 3);
temp_X = reshape(XYZ(:, 1), repmat(extent*range*n_points+1, [1, 2]));
pc(:, 1) = reshape(temp_X(1:range/4:end, 1:range/4:end), [], 1);
temp_Y = reshape(XYZ(:, 2), repmat(extent*range*n_points+1, [1, 2]));
pc(:, 2) = reshape(temp_Y(1:range/4:end, 1:range/4:end), [], 1);
temp_Z = reshape(XYZ(:, 3), repmat(extent*range*n_points+1, [1, 2]));
pc(:, 3) = reshape(temp_Z(1:range/4:end, 1:range/4:end), [], 1);

% Center the 3D point cloud at zero (center the surface data as well)
pc = pc - mean(pc, 1);
XYZ = XYZ - mean(XYZ, 1);
...
% Downsample the 3D point cloud according to downsample_factor using binning
num_bins = ceil(sqrt(size(pc, 1))/downsample_factor);
pc_downsampled = zeros(num_bins^2, 3);

temp_pcX = reshape(pc(:, 1), repmat(sqrt(size(pc, 1)), [1, 2]));
temp_pcY = reshape(pc(:, 2), repmat(sqrt(size(pc, 1)), [1, 2]));

pc_downsampled(:, 1) = reshape(temp_pcX(1:downsample_factor:end, 1:downsample_factor:end), [], 1);
pc_downsampled(:, 2) = reshape(temp_pcY(1:downsample_factor:end, 1:downsample_factor:end), [], 1);

temp_pcZ = reshape(pc(:, 3), repmat(sqrt(size(pc, 1)), [1, 2]));
temp_pcZ_downsampled = zeros(ceil(size(temp_pcZ)/4));

for i = 1:num_bins
    bin_i_start = (i-1) * downsample_factor + 1;
    bin_i_end = min(i * downsample_factor, sqrt(size(pc, 1)));
    for j = 1:num_bins
        bin_j_start = (j-1) * downsample_factor + 1;
        bin_j_end = min(j * downsample_factor, sqrt(size(pc, 1)));        
        temp_pcZ_downsampled(i, j) = mean(mean(temp_pcZ(bin_i_start:bin_i_end, bin_j_start:bin_j_end)));
    end
end

% pc_downsampled(:, 3) = reshape(temp_pcZ_downsampled, [], 1);
pc_downsampled(:, 3) = reshape(temp_pcZ(1:downsample_factor:end, 1:downsample_factor:end), [], 1);

%% Add some noise to simulate real world acquisition

% Add Gaussian nonise to the point cloud
noise_level = range/200; % Adjust the noise level as needed
noise = 0; %noise_level * randn(size(pc_downsampled)); % Generate Gaussian noise
...
angles = (rand(1, 3) * deg2rad(30)) - deg2rad(15);
...
t = rand(1, 3)*2*range-range; % Random translation in the range [0, 10]
...
figure; hold on;
surf(reshape(XYZ(:, 1), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
     reshape(XYZ(:, 2), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
     reshape(XYZ(:, 3), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
     'EdgeColor', 'none');
scatter3(reshape(pc(:, 1), repmat(sqrt(size(pc, 1)), [1, 2])), ...
         reshape(pc(:, 2), repmat(sqrt(size(pc, 1)), [1, 2])), ...
         reshape(pc(:, 3), repmat(sqrt(size(pc, 1)), [1, 2])), ...
...
scatter3(reshape(pc_transformed(:, 1), repmat(sqrt(size(pc_transformed, 1)), [1, 2])), ...
         reshape(pc_transformed(:, 2), repmat(sqrt(size(pc_transformed, 1)), [1, 2])), ...
         reshape(pc_transformed(:, 3), repmat(sqrt(size(pc_transformed, 1)), [1, 2])), ...
         'filled', 'MarkerFaceColor', 'r', 'SizeData', 5);
surf(reshape(XYZ_transformed(:, 1), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
     reshape(XYZ_transformed(:, 2), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
     reshape(XYZ_transformed(:, 3), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
     'EdgeColor', 'none');
for i = 1:4:24
...
if saveFigures
    ...
    exportgraphics(fig1, sprintf('%s/squiggly_surface.png', out_dir_LIGHT), 'Resolution', 300, 'Padding', 'figure');
    fig1.Theme = 'dark';
    fig1.Color = [0.15 0.15 0.15]; % figure background
    exportgraphics(fig1, sprintf('%s/squiggly_surface.png', out_dir_DARK), 'Resolution', 300, 'Padding', 'figure');
...
% Matrix to keep track of the total transformation
T = eye(4);
...
while (abs(prev_transformation_error - transformation_error) > eps) && (iteration - 1 < max_iterations)
    ...
    if saveFigures
        ...
        scatter3(reshape(Q(:, 1), repmat(sqrt(size(Q, 1)), [1, 2])), ...
                 reshape(Q(:, 2), repmat(sqrt(size(Q, 1)), [1, 2])), ...
                 reshape(Q(:, 3), repmat(sqrt(size(Q, 1)), [1, 2])), ...
                 'filled', 'MarkerFaceColor', 'r', 'SizeData', 5);
        surf(reshape(XYZ(:, 1), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
             reshape(XYZ(:, 2), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
             reshape(XYZ(:, 3), repmat(sqrt(size(XYZ, 1)), [1, 2])), ...
        ...
        % Input point cloud
        scatter3(reshape(P(:, 1), repmat(sqrt(size(P, 1)), [1, 2])), ...
                 reshape(P(:, 2), repmat(sqrt(size(P, 1)), [1, 2])), ...
                 reshape(P(:, 3), repmat(sqrt(size(P, 1)), [1, 2])), ...
                 'filled', 'MarkerFaceColor', 'r', 'SizeData', 5);
        surf(reshape(XYZ_transformed(:, 1), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
             reshape(XYZ_transformed(:, 2), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
             reshape(XYZ_transformed(:, 3), repmat(sqrt(size(XYZ_transformed, 1)), [1, 2])), ...
...
R_total = T(1:3, 1:3);
t_total = T(1:3, 4);
eul = rad2deg(rotm2eul(R_total, 'XYZ'));

fprintf("Total transformation - Rotation: %.2f°(X) %.2f°(Y) %.2f°(Z), Translation: %.2f, %.2f, %.2f\n", eul(1), eul(2), eul(3), t_total(1), t_total(2), t_total(3));
...
ylabel('y');
zlabel('z');
title(sprintf("Total transformation - Rotation: %.2f°(X) %.2f°(Y) %.2f°(Z), Translation: %.2f, %.2f, %.2f\n", eul(1), eul(2), eul(3), t_total(1), t_total(2), t_total(3)));